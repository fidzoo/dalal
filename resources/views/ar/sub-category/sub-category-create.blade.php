...
{!! Form::label('بنر القسم النسخة العربية') !!}
{!! Form::file('ar_banner') !!}<br>

{!! Form::label('بنر القسم النسخة الإنجليزية') !!}
...
<script>
	...
	var i= 2;
		$("#size").click(function(){
			if (i < 8) {
		$("#sizes").append('{!! Form::label("عنوان الحجم بالعربية") !!}<input type="text" name="ar_size'+i+'" class="form-control" required> <br> {!! Form::label("عنوان الحجم بالإنجليزية") !!}<input type="text" name="en_size'+i+'" class="form-control" required><br>');
			i++;
			}
			else{
	...
	var x= 2;
		$("#spec").click(function(){
			if (x < 6) {
		$("#specs").append('{!! Form::label("عنوان الوصف بالعربية") !!}<input type="text" name="ar_title'+x+'" class="form-control" required> <br> {!! Form::label("عنوان الوصف بالإنجليزية") !!}<input type="text" name="en_title'+x+'" class="form-control" required><br>');
			x++;
			}
			else{
				alert('لقد وصلت للعدد المسموح به');
