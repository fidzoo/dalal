        ...
        <div class="panel">
            ...
            <div class="panel-heading">
                <h4 class="panel-title">
                        ...
                        المتجر: <strong>{{$order->store->ar_name}}</strong><br><br>
                        حالة الطلب: @if($order->order_status == 0)
                                    <strong>في المراجعة</strong>
                                  @elseif($order->order_status == 1)
                                    <strong>تم شحن جزء من الطب</strong>
                                  @elseif($order->order_status == 2)
                                    <strong>تم شحن الطلب بالكامل</strong>
                                  @elseif($order->order_status == 3)
                                    <strong>تم توصيل حزء من الطلب</strong>
                                  @elseif($order->order_status == 4) 
                                    <strong>تم تسليم الطلب بالكامل</strong>
                                  @elseif($order->order_status == 5)
                                    <strong>جاري استرجاع الطلب</strong>
                                  @endif  
                    ...
                    <tr>
                        ...
                        <td>
                            <select id="status{{$no}}{{$p_no}}" name="status{{$no}}{{$p_no}}">
                                ...
                                <option value="0" {{$item->delivery_status == 0 ? 'selected' : ''}}>مراجعة الطلب</option>
                                <option value="1" {{$item->delivery_status == 1 ? 'selected' : ''}}>تم الشحن</option>
